...
@given(u'Escribo el usuario "{usuario}" y la contraseña "{contra}"')
...
@when(u'cliqueo el botón identificarse')
def step_impl(context):
    context.driver.find_element(By.XPATH, '//*[@id="submit"]').click()
    time.sleep(2)


@when(u'puedo ver la página de inicio')
def step_impl(context):
...
@when(u'Presiono alguna de las categoria de los convenios')
def step_impl(context):
...
@when(u'presiono el boton + persona')
def step_impl(context):
...
@when(u'presiono save en la persona')
def step_impl(context):
    context.driver.find_element(
...
@when(u'presiono nuevamente la categoria del convenio')
def step_impl(context):
    context.driver.find_element(
        By.XPATH, '/html/body/nav/div/div[2]/div/ul[1]/li[2]').click()
    time.sleep(2)
...
@when(u'presiono en el boton + unidad academica')
def step_impl(context):
    context.driver.find_element(By.XPATH, '/html/body/div[2]/a[3]').click()
    time.sleep(2)


@when(u'ingresooo el nombre "{nombre}"')
def step_impl(context, nombre):
    context.driver.find_element(By.NAME, 'nombre').send_keys(nombre)
    time.sleep(2)


@when(u'presiono save en la categoria')
def step_impl(context):
    context.driver.find_element(
        By.XPATH, '//*[@id="panel"]/div/form/button').click()
    time.sleep(2)
...
@when(u'presiono por ultima vez la categoria del convenio')
def step_impl(context):
    context.driver.find_element(
        By.XPATH, '/html/body/nav/div/div[2]/div/ul[1]/li[2]').click()
    time.sleep(2)
...
@then(u'obtengo el mensajee "{mensaje}"')
def step_impl(context, mensaje):
    assert mensaje in context.driver.page_source
    # Eliminar convenio
    context.driver.find_element(
